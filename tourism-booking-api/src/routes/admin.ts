...
router.get('/sync/status', async (req: Request, res: Response) => {
  try {
    const organizationId = BigInt(req.query.organization_id as string || '1');

    const status = await PleasanterUserSyncService.getSyncStatus(organizationId);
    const pendingReconciliation = await PleasanterUserSyncService.getReconciliationStatus(
      organizationId,
      10
    );
...
router.post('/sync/users', async (req: SyncRequest, res: Response) => {
  try {
    ...
    // Execute sync
    const result = await PleasanterUserSyncService.syncUsers(
      organizationId,
      pleasanterUsers,
      dryRun
    );
...
router.get('/sync/reconciliation', async (req: Request, res: Response) => {
  try {
    const organizationId = BigInt(req.query.organization_id as string || '1');
    const limit = parseInt(req.query.limit as string) || 50;

    const logs = await PleasanterUserSyncService.getReconciliationStatus(
      organizationId,
      limit
    );
