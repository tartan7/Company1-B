...
import { AuditService, AuditLogQuery } from '../services/auditService';
...
router.get('/', verifyToken, async (req: AuditRequest, res: Response): Promise<void> => {
  try {
    const { startDate, endDate, operationType, resourceType, actorId, resourceId, limit, offset } =
      req.query;
...
router.get('/export/csv', verifyToken, async (req: AuditRequest, res: Response): Promise<void> => {
  try {
...
router.get('/:id', verifyToken, async (req: AuditRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
