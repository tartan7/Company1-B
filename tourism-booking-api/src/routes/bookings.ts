 ...
 */
router.post('/', verifyToken, validateBooking, async (req: Request, res: Response) => {
  try {
    const { scheduleId, activityId, quantity, bookingDate, notes } = req.body;
    const guestId = BigInt((req as any).user?.id);
 ...
 */
router.patch('/:id/confirm', verifyToken, async (req: Request, res: Response) => {
  try {
    const bookingId = BigInt(req.params.id);
    const userId = BigInt((req as any).user?.id);
 ...
 */
router.get('/schedules/:scheduleId/availability', async (req: Request, res: Response) => {
  try {
