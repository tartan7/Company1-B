...
import { ActivityService, SearchFilters } from '../services/activityService';
  ...
  try {
    ...
    if (req.query.sortBy && typeof req.query.sortBy === 'string') {
      ...
      if (validSortBy.includes(req.query.sortBy)) {
        filters.sortBy = req.query.sortBy as SearchFilters['sortBy'];
      }
    }
...
router.post('/', verifyToken, requireOperator, (req: AuthRequest, res) => {
  try {
...
router.patch('/:id', verifyToken, requireOperator, (req: AuthRequest, res) => {
  try {
    const input: UpdateActivityInput = req.body;
...
router.delete('/:id', verifyToken, requireOperator, (req: AuthRequest, res) => {
  try {
    const userId = typeof req.userId === 'string' ? req.userId : req.userId?.[0];
    if (!userId) {
      res.status(401).json({ error: 'User ID not found' });
...
router.delete('/:id/images/:s3Key', verifyToken, requireOperator, async (req: AuthRequest, res) => {
  try {
    const userId = typeof req.userId === 'string' ? req.userId : req.userId?.[0];
    if (!userId) {
      res.status(401).json({ error: 'User ID not found' });
