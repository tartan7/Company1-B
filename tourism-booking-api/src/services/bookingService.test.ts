...
import { schedules, bookings, activities, users } from '../db/schema';
...
test('BookingService - Transaction Isolation & Row-Level Locking', async (t) => {
  ...
  await t.test('should successfully book available slots', async () => {
    const result = await BookingService.bookSlots(
      testScheduleId,
      guestId,
      testActivityId,
      3,
  ...
  await t.test('should maintain ACID properties during concurrent bookings', async () => {
    ...
    assert.strictEqual(
      successful.length,
      5,
      `Should have exactly 5 successful bookings, got ${successful.length}`
    );
    assert.strictEqual(
      failed.length,
      5,
      `Should have exactly 5 failed bookings, got ${failed.length}`
    );
    ...
    assert.strictEqual(
      updatedSchedule?.bookedSlots,
      5,
      `Schedule should show 5 booked slots, showing ${updatedSchedule?.bookedSlots}`
    );
  });
  ...
  await t.test('should release slots when booking is cancelled', async () => {
    ...
    // Book 5 slots
    const bookingResult = await BookingService.bookSlots(
      cancelScheduleId,
      guestId,
      testActivityId,
      5,
