...
export class ImageService {
  ...
  private generateS3Key(activityId: string, variant: 'original' | 'optimized' | 'thumbnail'): string {
    const timestamp = Date.now();
    return `activities/${activityId}/${variant}_${timestamp}.webp`;
  }

  async uploadToS3(
    processedImages: { original: Buffer; optimized: Buffer; thumbnail: Buffer },
    activityId: string,
    ...
    const uploadImage = async (buffer: Buffer, variant: 'original' | 'optimized' | 'thumbnail') => {
  ...
  async uploadImage(file: Express.Multer.File, activityId: string): Promise<ImageUploadResult> {
    // Validate image
