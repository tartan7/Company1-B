...
export class BookingService {
  ...
  static async bookSlots(
    ...
    try {
      ...
      const bookingResult = await client.query(
        `INSERT INTO bookings (guest_id, schedule_id, activity_id, quantity, booking_date, status, created_at, updated_at)
         VALUES ($1, $2, $3, $4, $5, 'pending', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
         RETURNING id`,
        [guestId, scheduleId, activityId, quantity, bookingDate]
      );
   ...
   */
  static async cancelBooking(bookingId: bigint): Promise<BookingResult> {
    const client = await getClient();
    ...
    try {
      ...
      // Get booking details
      const bookingResult = await client.query(
        'SELECT id, schedule_id, quantity, status FROM bookings WHERE id = $1 FOR UPDATE',
        [bookingId]
      );
   ...
   */
  static async getAvailableSlots(scheduleId: bigint): Promise<AvailabilitySummary | null> {
    const client = await getClient();
   ...
   */
  static async getBooking(bookingId: bigint) {
   ...
   */
  static async getGuestBookings(guestId: bigint) {
   ...
   */
  static async getScheduleBookings(scheduleId: bigint) {
    return await db
      .select()
      .from(bookings)
   ...
   */
  static async confirmBooking(bookingId: bigint): Promise<BookingResult> {
    const client = await getClient();
  ...
  static async publishBooking(bookingId: bigint, publishedBy: string): Promise<PublishResult> {
