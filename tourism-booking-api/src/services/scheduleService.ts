...
import { SnapshotService, PublishResult } from './snapshotService';

const DEFAULT_TIMEZONE = 'UTC';

function mapDbScheduleToSchedule(dbSchedule: any): Schedule {
...
export class ScheduleService {
  ...
  static async createSchedule(input: CreateScheduleInput, operatorId: string): Promise<Schedule> {
  ...
  static async getSchedule(id: string, userTimezone?: string): Promise<Schedule | null> {
    const result = await db
      .select()
      .from(schedules)
      .where(and(
        eq(schedules.id, BigInt(id)),
  ...
  static async getSchedulesByActivity(activityId: string, availabilityOnly = false): Promise<Schedule[]> {
    let query = db
      .select()
      .from(schedules)
      .where(and(
        eq(schedules.activityId, BigInt(activityId)),
        eq(schedules.isDeleted, false)
      ));
  ...
  static async updateSchedule(id: string, input: UpdateScheduleInput, operatorId: string): Promise<Schedule | null> {
    ...
    if (input.totalSlots !== undefined) updates.totalSlots = input.totalSlots;
  ...
  static async deleteSchedule(id: string, operatorId: string): Promise<boolean> {
    const current = await db
      .select()
      .from(schedules)
      .where(and(
  ...
  static async bookSlots(scheduleId: string, slotsToBook: number): Promise<Schedule> {
    return await withTransaction(async (tx) => {
      ...
      try {
        ...
        const updateResult = await client.query(
          'UPDATE schedules SET booked_slots = booked_slots + $1, updated_at = NOW() WHERE id = $2 RETURNING *',
          [slotsToBook, BigInt(scheduleId)]
        );
  ...
  static async releaseSlots(scheduleId: string, slotsToRelease: number): Promise<Schedule> {
    return await withTransaction(async (tx) => {
      const client = await getClient();
      try {
        // Lock the row with SELECT...FOR UPDATE
        ...
        const updateResult = await client.query(
          'UPDATE schedules SET booked_slots = booked_slots - $1, updated_at = NOW() WHERE id = $2 RETURNING *',
          [releasedSlots, BigInt(scheduleId)]
        );
  ...
  static async publishSchedule(scheduleId: string, publishedBy: string): Promise<PublishResult> {
    const current = await db
      .select()
      .from(schedules)
      .where(and(
        eq(schedules.id, BigInt(scheduleId)),
        eq(schedules.isDeleted, false)
      ));
  ...
  static async getScheduleSummaries(activityId: string): Promise<any[]> {
  ...
  static async getScheduleDetails(scheduleId: string): Promise<any | null> {
    ...
    const bookingList = await db
      ...
      .from(bookings)
      .where(and(
        eq(bookings.scheduleId, BigInt(scheduleId)),
  ...
  static async listActivitySummaries(): Promise<any[]> {
    ...
    const summaries = await Promise.all(
      activities.map(async (activity) => {
        const scheduleList = await db
          ...
          .from(schedules)
          .where(and(
            eq(schedules.activityId, activity.activityId),
            eq(schedules.isDeleted, false)
          ));
