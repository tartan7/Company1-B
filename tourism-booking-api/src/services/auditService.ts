import { AuditLog, AuditLogInsert } from '../db/schema.js';
import { db } from '../db/index.js';
import { auditLogs } from '../db/schema.js';
import { eq, and, gte, lte, desc } from 'drizzle-orm';

export interface AuditLogEntry {
  operationType: 'create' | 'update' | 'delete' | 'sync' | 'export' | 'query' | 'publish' | 'restore' | 'delete_version';
  resourceType: string;
  resourceId: string;
...
export class AuditService {
  static async logOperation(entry: AuditLogEntry): Promise<AuditLog> {
  ...
  static async queryAuditLogs(query: AuditLogQuery): Promise<{ logs: AuditLog[]; total: number }> {
  ...
  static async getAuditLogById(id: bigint): Promise<AuditLog | null> {
    const result = await db.select().from(auditLogs).where(eq(auditLogs.id, id));
    return result[0] || null;
  }

  static async getResourceAuditHistory(resourceType: string, resourceId: string): Promise<AuditLog[]> {
    return await db
      .select()
      .from(auditLogs)
