...
import { parseISO, isValid, getTime, parse, format } from 'date-fns';
...
export class TimezoneService {
  ...
  static convertToUTC(dateStr: string, timeStr: string, timezone: string): Date {
  ...
  static convertFromUTC(utcDate: Date, timezone: string): { date: string; time: string } {
    if (!this.validateTimezone(timezone)) {
      throw new Error(`Unsupported timezone: ${timezone}`);
    }
  ...
  static getDateRangeUTC(startDate: string, endDate: string, timeStr: string, timezone: string): TimezoneRange {
  ...
  static getUTCOffset(date: Date, timezone: string): number {
  ...
  static isAmbiguousTime(dateStr: string, timeStr: string, timezone: string): boolean {
    if (!this.validateTimezone(timezone)) {
      throw new Error(`Unsupported timezone: ${timezone}`);
    }
  ...
  static isValidDateTimeInTimezone(dateStr: string, timeStr: string, timezone: string): boolean {
    try {
  ...
  static isMidnightCrossing(startDate: string, endDate: string): boolean {
    return startDate !== endDate;
  }

  static handleDSTSpringForward(dateStr: string, timeStr: string, timezone: string): { date: string; time: string } {
    if (!this.validateTimezone(timezone)) {
      throw new Error(`Unsupported timezone: ${timezone}`);
    }
  ...
  static handleDSTFallBack(dateStr: string, timeStr: string, timezone: string, isFirstOccurrence: boolean = true): { date: string; time: string } {
    if (!this.validateTimezone(timezone)) {
      throw new Error(`Unsupported timezone: ${timezone}`);
    }
  ...
  static compareTimesAcrossTimezones(
    date1Str: string,
    time1Str: string,
    tz1: string,
    date2Str: string,
