...
test('Publish Workflow - Schedule and Booking Publishing', async (t) => {
  ...
  await t.test('setup - create test users and activity', async () => {
    ...
    const userResult = await db
      ...
      .returning();
    testUserId = (userResult[0] as any).id as bigint;
    ...
    const guestResult = await db
      ...
      .returning();
    guestId = (guestResult[0] as any).id as bigint;
    ...
    const activityResult = await db
      ...
      .returning();
    testActivityId = (activityResult[0] as any).id as bigint;
  });
  ...
  await t.test('should publish a schedule', async () => {
    ...
    const scheduleResult = await db
      ...
      .returning();
    testScheduleId = (scheduleResult[0] as any).id as bigint;
  ...
  await t.test('should create snapshot in resource_versions', async () => {
    const versions = await db
      ...
      .where(
        and(
          eq(resourceVersions.resourceType, 'schedule'),
          eq(resourceVersions.resourceId, testScheduleId)
        )
      );
  ...
  await t.test('should prevent updates to published schedule', async () => {
    const updateAttempt = await ScheduleService.updateSchedule(
      testScheduleId.toString(),
      { totalSlots: 25 },
      testUserId.toString()
    );
  ...
  await t.test('should publish a confirmed booking', async () => {
    ...
    const newScheduleResult = await db
      ...
      .returning();
    const newScheduleId = (newScheduleResult[0] as any).id as bigint;
    ...
    const bookingResult = await db
      ...
      .returning();
    testBookingId = (bookingResult[0] as any).id as bigint;
  ...
  await t.test('should allow publishing only confirmed bookings', async () => {
    ...
    const pendingBookingResult = await db
      ...
      .returning();
    const pendingBookingId = (pendingBookingResult[0] as any).id as bigint;

    const publishResult = await BookingService.publishBooking(
      pendingBookingId,
      testUserId.toString()
    );
