...
async function runLoadTest(): Promise<LoadTestResult> {
  ...
  const bookingPromises = guestIds.map((guestId, index) => {
    ...
    return BookingService.bookSlots(
      scheduleId,
      guestId,
      activityId,
      1, // Each guest books 1 slot
      '2026-07-01'
    ).then((result) => {
  ...
  if (successfulBookings > 100) {
    console.error(
      '❌ ERROR: More than 100 bookings succeeded! Overbooking detected!'
    );
  ...
  console.log(
    `  Race Condition Detected:  ${raceConditionDetected ? '❌ YES' : '✅ NO'}`
  );
  ...
  console.log(
    `  Consistency:  ${finalSchedule?.bookedSlots === successfulBookings ? '✅ CORRECT' : '❌ MISMATCH'}`
  );
