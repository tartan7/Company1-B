...
async function runLoadTest() {
  ...
  try {
    ...
    const bookingPromises1 = Array.from({ length: 100 }, () =>
    ...
    const bookingPromises2 = Array.from({ length: 200 }, () =>
      ScheduleService.bookSlots(schedule.id, 2).catch(() => null)
    );
    ...
    const mixedPromises = [
      ...
      ...Array.from({ length: 75 }, () =>
        ScheduleService.releaseSlots(schedule.id, 1).catch(() => null)
      ),
    ];
