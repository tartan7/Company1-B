...
describe('Activities Routes - Publish Endpoint', () => {
    ...
    beforeEach(async () => {
        ...
        try {
         ...
         RETURNING id`, [testUserId, 'Test Activity', 'Test Description', 'Adventure', '99.99', 'USD', 10, 60, 'Test Location', 'active']);
            testActivityId = BigInt(activityResult.rows[0].id);
            await client.query('COMMIT');
            // Create auth token
            authToken = jwt.sign({ userId: testUserId.toString(), role: 'operator' }, process.env.JWT_SECRET || 'secret');
        }
        finally {
            client.release();
        }
