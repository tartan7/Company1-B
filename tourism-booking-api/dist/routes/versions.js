...
router.post('/:resourceType/:id/publish', verifyToken, requireOperator, async (req, res) => {
    try {
        ...
        const result = await SnapshotService.publishEntity(resourceType, BigInt(id), BigInt(userId));
        if (!result.success) {
...
router.get('/:resourceType/:id/versions', async (req, res) => {
    try {
        const resourceType = Array.isArray(req.params.resourceType) ? req.params.resourceType[0] : req.params.resourceType;
        const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
        if (!isValidResourceType(resourceType)) {
...
router.get('/:resourceType/:id/versions/:version', async (req, res) => {
    try {
        const resourceType = Array.isArray(req.params.resourceType) ? req.params.resourceType[0] : req.params.resourceType;
        const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
...
router.get('/:resourceType/:id/current-version', async (req, res) => {
    try {
        const resourceType = Array.isArray(req.params.resourceType) ? req.params.resourceType[0] : req.params.resourceType;
        const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
        if (!isValidResourceType(resourceType)) {
...
router.patch('/:resourceType/:id/versions/:version/delete', verifyToken, requireOperator, async (req, res) => {
    try {
        const resourceType = Array.isArray(req.params.resourceType) ? req.params.resourceType[0] : req.params.resourceType;
        const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
        const versionStr = Array.isArray(req.params.version) ? req.params.version[0] : req.params.version;
        ...
        const result = await SnapshotService.softDeleteVersion(resourceType, BigInt(id), versionNum, BigInt(userId));
        if (!result.success) {
...
router.post('/:resourceType/:id/versions/:version/restore', verifyToken, requireOperator, async (req, res) => {
    try {
        const resourceType = Array.isArray(req.params.resourceType) ? req.params.resourceType[0] : req.params.resourceType;
        const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
        const versionStr = Array.isArray(req.params.version) ? req.params.version[0] : req.params.version;
        ...
        const result = await SnapshotService.restoreVersion(resourceType, BigInt(id), versionNum, BigInt(userId));
        if (!result.success) {
            if (result.error === 'VERSION_NOT_FOUND') {
                res.status(404).json({ error: result.message });
            }
