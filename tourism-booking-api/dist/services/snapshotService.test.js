...
import { activities, schedules, resourceVersions } from '../db/schema';
...
describe('SnapshotService', () => {
    ...
    beforeEach(async () => {
        ...
        try {
            ...
            // Create test user
            const userResult = await client.query(`INSERT INTO users (email, password_hash, first_name, last_name, created_at, updated_at)
         VALUES ($1, $2, $3, $4, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
         RETURNING id`, ['test@example.com', 'hash', 'Test', 'User']);
            testUserId = BigInt(userResult.rows[0].id);
            // Create test activity
            const activityResult = await client.query(`INSERT INTO activities (host_id, title, description, category, price_per_person, currency, max_capacity, duration, location, created_at, updated_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
         RETURNING id`, [testUserId, 'Test Activity', 'Test Description', 'Adventure', '99.99', 'USD', 10, 60, 'Test Location']);
            testActivityId = BigInt(activityResult.rows[0].id);
            // Create test schedule
            const scheduleResult = await client.query(`INSERT INTO schedules (activity_id, start_date, end_date, start_time, end_time, timezone, total_slots, operator_id, created_at, updated_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
         RETURNING id`, [testActivityId, '2026-06-01', '2026-06-01', '09:00', '10:00', 'UTC', 10, testUserId]);
            testScheduleId = BigInt(scheduleResult.rows[0].id);
            // Create test booking
            const bookingResult = await client.query(`INSERT INTO bookings (guest_id, schedule_id, activity_id, quantity, booking_date, created_at, updated_at)
         VALUES ($1, $2, $3, $4, $5, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
         RETURNING id`, [testUserId, testScheduleId, testActivityId, 2, '2026-06-01']);
    ...
    describe('publishEntity', () => {
        ...
        it('should include activity summary in snapshot', async () => {
            ...
            try {
                ...
                // Create 3 schedules
                for (let i = 0; i < 3; i++) {
                    await client.query(`INSERT INTO schedules (activity_id, start_date, end_date, start_time, end_time, timezone, total_slots, booked_slots, operator_id, created_at, updated_at)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`, [testActivityId, '2026-06-01', '2026-06-01', '09:00', '10:00', 'UTC', 10 + i, i, testUserId]);
                }
                await client.query('COMMIT');
            }
            finally {
        ...
        it('should compute correct schedule summary with partial bookings', async () => {
            ...
            const client = await getClient();
            try {
                await client.query(`UPDATE schedules SET booked_slots = 5 WHERE id = $1`, [testScheduleId]);
            }
            finally {
                client.release();
            }
        ...
        it('should compute correct schedule summary when fully booked', async () => {
            ...
            const client = await getClient();
            try {
                await client.query(`UPDATE schedules SET booked_slots = 10 WHERE id = $1`, [testScheduleId]);
            }
            finally {
                client.release();
            }
        ...
        it('should correctly count available schedules in activity summary', async () => {
            ...
            try {
                ...
                const schedule1 = await client.query(`INSERT INTO schedules (activity_id, start_date, end_date, start_time, end_time, timezone, total_slots, booked_slots, operator_id, created_at, updated_at)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
           RETURNING id`, [testActivityId, '2026-06-02', '2026-06-02', '09:00', '10:00', 'UTC', 5, 5, testUserId]);
                // Partially booked schedule
                const schedule2 = await client.query(`INSERT INTO schedules (activity_id, start_date, end_date, start_time, end_time, timezone, total_slots, booked_slots, operator_id, created_at, updated_at)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
           RETURNING id`, [testActivityId, '2026-06-03', '2026-06-03', '09:00', '10:00', 'UTC', 8, 3, testUserId]);
                // Empty schedule
                const schedule3 = await client.query(`INSERT INTO schedules (activity_id, start_date, end_date, start_time, end_time, timezone, total_slots, booked_slots, operator_id, created_at, updated_at)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
           RETURNING id`, [testActivityId, '2026-06-04', '2026-06-04', '09:00', '10:00', 'UTC', 10, 0, testUserId]);
                await client.query('COMMIT');
            }
            finally {
                client.release();
    ...
    describe('summary edge cases', () => {
        it('should handle schedule with zero slots', async () => {
            const client = await getClient();
            let zeroSlotScheduleId;
            try {
                await client.query('BEGIN');
                const result = await client.query(`INSERT INTO schedules (activity_id, start_date, end_date, start_time, end_time, timezone, total_slots, booked_slots, operator_id, created_at, updated_at)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
           RETURNING id`, [testActivityId, '2026-06-05', '2026-06-05', '09:00', '10:00', 'UTC', 0, 0, testUserId]);
            ...
            const publishResult = await SnapshotService.publishEntity('schedule', zeroSlotScheduleId, testUserId);
            expect(publishResult.success).toBe(true);
            const snapshot = await SnapshotService.getVersion('schedule', zeroSlotScheduleId, 2);
        ...
        it('should compute accurate percentage for various booking levels', async () => {
            ...
            for (let i = 0; i < testCases.length; i++) {
                const publishResult = await SnapshotService.publishEntity('schedule', scheduleIds[i], testUserId);
                expect(publishResult.success).toBe(true);
